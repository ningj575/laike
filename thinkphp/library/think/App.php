...
class App extends Container
{
    ...
    protected function loadLangPack()
    {
        ...
        ],$this->getDirFile($this->appPath. DIRECTORY_SEPARATOR.$this->bindModule.DIRECTORY_SEPARATOR. 'lang' . DIRECTORY_SEPARATOR . $this->request->langset()),
          $this->getDirFile($this->appPath . 'lang' . DIRECTORY_SEPARATOR . $this->request->langset())
            )
        );
    }
     ...
     * 获取文件夹下的文件
     */
    protected function getDirFile($dir){
        if(!is_dir($dir)){
            return [];
        }
        $conname = array();
        ...
        foreach ($file_names as $k => $v) {
            ...
            $conname[] = $dir.DIRECTORY_SEPARATOR.$v;
        }
        return $conname;
    }
