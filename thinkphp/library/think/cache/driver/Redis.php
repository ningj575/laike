...
class Redis extends Driver
{
     ...
     * @return boolean
     */
    public function setLock($name,$expire = 20)
    {
        $key = 'redis_Lock:'.$name;
        $isLock = $this->handler->setnx($key,1);
        if( !$isLock ){
            //本来这里没必要处理了，但是为了防止特殊情况，锁设置成功，但是Redis::expire设置缓存时间失败，这样会导致死锁
            if( $this->handler->ttl($key ) == -1 ){
   ...
   /**
     ...
     * @param $key
     * @param $value
     * @param $sort 出队顺序 1--先进先出；2--先进后出
     * @return int
     */
    public function lpush($key, $value, $sort = 1)
    {
        if($sort == 2){
